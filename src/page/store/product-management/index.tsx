...
export const ProductManagementPage: React.FC = () => {
  ...
  const [isAddProductBottomSheetOpen, setIsAddProductBottomSheetOpen] =
    useState(false);
  const [isEditProductBottomSheetOpen, setIsEditProductBottomSheetOpen] =
    useState(false);
  const { openModal, closeModal } = useModalStore();
  const [selectedAddProduct, setSelectedAddProduct] =
  ...
  const deleteProductMutation = useMutation({
    mutationFn: (productId: string) =>
      deleteStoreProduct({ childId: selectedChildId || "", productId }),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["storeItems", selectedChildId],
      });
  ...
  useEffect(() => {
    ...
    const productItems: ProductItem[] = storeItems.map(
      (item: __ProductItem) => ({
        productId: item.id,
  ...
  return (
    <>
      ...
      {/* 상품 추가 Bottom Sheet */}
      <BottomSheet
        isOpen={isAddProductBottomSheetOpen}
      ...
      <BottomSheet
        ...
        onClose={() => setIsEditProductBottomSheetOpen(false)}
      >
        {/* 제목 */}
